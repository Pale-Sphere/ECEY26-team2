...
// staging register, to avoid coincidence bug with ce_synch & theta_valid, but also prevents reading mid-update
reg [(TOTAL_WIDTH - 1):0] next_frame; //can be written anytime
reg new_data_ready; 
...
wire sclk_r = (sclk_d == 0 && sclk_synch == 1); //rising edge of raspberry pi's clock and chip enable
wire ce_r = (ce_d == 0 && ce_synch == 1);
...
always @ (posedge clk or posedge rst) begin
    if (rst) begin
        ...
        if (theta_valid && !new_data_ready /* && ce_synch*/)  begin //trying to remove ce_synch
            next_frame <= {HEADER, theta}; //store theta into the latched register while a transmission isn't happening and add the header
            new_data_ready <= 1'b1;
        end
    
        else if (ce_r && new_data_ready) begin
            //Since system is idiling, can update transmission frame
            frame_reg <= next_frame;
            // Clear flag when SPI consumes it
            new_data_ready <= 1'b0;
        end
    end
end
