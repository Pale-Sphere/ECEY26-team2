...
//   I2S mics → IBUFG → I2S receivers → async FIFOs → sample gates
...
//   bck_0_buf   - BCK from mic pair 0 (via IBUFG)
//   bck_1_buf   - BCK from mic pair 1 (via IBUFG)
//
...
module top (
    ...
    wire clk_24mhz;
    wire mmcm_locked;
    wire sclk_I2S_0_unused, sclk_I2S_1_unused;
 
    clk_wiz_0 clk_wiz_inst (
        .clk_in1  (clock),
        .reset    (rst_btn),
        .clk_100MHz (clk_100mhz),
        .clk_24MHz_0 (sclk_I2S_0_unused),
        .clk_24MHz_1 (sclk_I2S_1_unused)
    );
    ...
    // Fixed test pattern replacing I2S receivers
    // p0l=0x400000  p0r=0x100000  → Y = log2(4) = 2.0
    // p1l=0x400000  p1r=0x200000  → X = log2(2) = 1.0
    // expected angle = atan2(2.0, 1.0) = 63.43 degrees
    wire [23:0] pair0_ldata = 24'h400000;
    wire [23:0] pair0_rdata = 24'h100000;
    wire [23:0] pair1_ldata = 24'h400000;
    wire [23:0] pair1_rdata = 24'h200000;

    // Sample ready strobe at ~48 kHz (every 2083 100MHz cycles)
    localparam SAMPLE_PERIOD = 11'd2083;
    reg [10:0] sample_tick_ctr;
    reg        test_sample_ready;
    always @(posedge clk_100mhz or posedge rst_sys) begin
        if (rst_sys) begin
            sample_tick_ctr   <= 11'd0;
            test_sample_ready <= 1'b0;
        end else begin
            test_sample_ready <= 1'b0;
            if (sample_tick_ctr == SAMPLE_PERIOD - 1) begin
                sample_tick_ctr   <= 11'd0;
                test_sample_ready <= 1'b1;
            end else begin
                sample_tick_ctr <= sample_tick_ctr + 11'd1;
            end
        end
    end

    wire pair0_sample_ready = test_sample_ready;
    wire pair1_sample_ready = test_sample_ready;
    ...
    //  FIFO instantiations
    CDC_FIFO fifo_p0l (
        .rst(rst_sys), .wr_clk(clk_100mhz), .rd_clk(clk_100mhz),
    ...
    rms_block rms_p0l_inst (
        .clk(clk_100mhz), .rst(rst_sys),
        .sample_in(gated_p0l), .sample_valid(gated_p0l_valid),
        .rms_out(rms_p0l), .rms_valid(rms_p0l_valid)
    );
    rms_block rms_p0r_inst (
        .clk(clk_100mhz), .rst(rst_sys),
        .sample_in(gated_p0r), .sample_valid(gated_p0r_valid),
        .rms_out(rms_p0r), .rms_valid(rms_p0r_valid)
    );
    rms_block rms_p1l_inst (
        .clk(clk_100mhz), .rst(rst_sys),
        .sample_in(gated_p1l), .sample_valid(gated_p1l_valid),
        .rms_out(rms_p1l), .rms_valid(rms_p1l_valid)
    );
    rms_block rms_p1r_inst (
        .clk(clk_100mhz), .rst(rst_sys),
        .sample_in(gated_p1r), .sample_valid(gated_p1r_valid),
