...
module top (
    ...
    // Treating bck as an input in this implementation to help with timing issues
...
//    //Testing bridge synchonization as alternative to FIFO 
//    wire [23:0] pair0l_sync, pair0r_sync;
//    wire        p0_valid_sync;

//    i2s_to_rms_bridge bridge0 (
//        .clk(clk_100mhz),
//        .rst(rst_sys),

//        .ldata_in(pair0_ldata),
//        .rdata_in(pair0_rdata),
//        .sample_ready_in(pair0_sample_ready),

//        .ldata_out(pair0l_sync),
//        .rdata_out(pair0r_sync),
//        .sample_valid_out(p0_valid_sync)
//    );
    
//    wire [23:0] pair1l_sync, pair1r_sync;
//    wire        p1_valid_sync;

//    i2s_to_rms_bridge bridge1 (
//        .clk(clk_100mhz),
//        .rst(rst_sys),

//        .ldata_in(pair1_ldata),
//        .rdata_in(pair1_rdata),
//        .sample_ready_in(pair1_sample_ready),

//        .ldata_out(pair1l_sync),
//        .rdata_out(pair1r_sync),
//        .sample_valid_out(p1_valid_sync)
//    );
...
//    wire test_enable1r = 1'b1;
    ...
    rms_block rms_p0l_inst (
        .clk(clk_100mhz), .rst(rst_sys),
        .sample_in(pair0_ldata), .sample_valid(pair0_sample_ready),
        .rms_out(rms_p0l), .rms_valid(rms_p0l_valid)
    );
    rms_block rms_p0r_inst (
        .clk(clk_100mhz), .rst(rst_sys),
        .sample_in(pair0_rdata), .sample_valid(pair0_sample_ready),
        .rms_out(rms_p0r), .rms_valid(rms_p0r_valid)
    );
    rms_block rms_p1l_inst (
        .clk(clk_100mhz), .rst(rst_sys),
        .sample_in(pair1_ldata), .sample_valid(pair1_sample_ready),
        .rms_out(rms_p1l), .rms_valid(rms_p1l_valid)
    );
    rms_block rms_p1r_inst (
        .clk(clk_100mhz), .rst(rst_sys),
        .sample_in(pair1_rdata), .sample_valid(pair1_sample_ready),
...
//    // Pipelined difference generator for angle_calc
...
//        end else begin
//            if (log2_sync_valid) begin
//                y_diff <= $signed(log2_hold_p0l) - $signed(log2_hold_p0r);
//                x_diff <= $signed(log2_hold_p1l) - $signed(log2_hold_p1r);
//                diff_valid <= 1'b1;
//            end else begin
//                diff_valid <= 1'b0;
//            end
...
//    end
    ...
    //concatenating 0's with output angle (might have to shift in python code)
    reg [23:0] theta_reg;
    reg        theta_valid_reg;
    ...
    always @(posedge clk_100mhz or posedge rst_sys) begin
        if (rst_sys) begin
            ...
            theta_reg <= {8'b0, angle_out};
            theta_valid_reg <= angle_valid;
        end
    end
